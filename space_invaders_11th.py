...
def add_enemies(number_of_ens,speed):
    ...
    for i in range(number_of_ens):
        ...
        if i <=8:
            ...
            x_1+=1
            enemyX.append((x_1)*64)
            enemyY.append(125)
        elif i>=17 and i<=24:
            x_2+=1
            enemyX.append((x_2)*64)
            enemyY.append(200)
        elif i>=25 and i<=356:
            x_3+=1
            enemyX.append((x_3)*64)
            enemyY.append(275)
...
def win_check():
    global fuck, win_count, time_counter
    dead_counter=0
    for i in enemyY:
        if i <=0:
            dead_counter+=1
    if dead_counter == (len(enemyX)):
        win_count+=1
        fuck = True
...
while running:
    ...
    enemyX = [ x + v for (v,x) in zip(enemyX, enemyX_change)]
...
#i think the line enemyImg, enemyX, enemyY, enemyX_change, enemyY_change = [],[],[],[],[] 
#shoulb just after global enemyImg, enemyX, enemyY, enemyX_change, enemyY_change,number_of_enemies like that you clean the lists at each lvl
#and instead of number_of_enemies you should use len(enemyX)
